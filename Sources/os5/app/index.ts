...
const _ampm1 = _ampmContainer1.getElementsByTagName("image");
const _ampm2 = _ampmContainer2.getElementsByTagName("image");
...
const _hrmlImage = _hrmIcon.getElementById("icon") as ImageElement;
const _hrmTexts = document.getElementById("hrm-text-container").getElementsByTagName("image");
...
simpleMinutes.initialize("user", false, 0, (clock1, clock2, mins) => {
...
simpleSettings.initialize(
  ...
  (settingsNew: Settings) => {
    ...
    if (settingsNew.offset !== undefined) {
      try {
        const value: string = settingsNew.offset.values[0].value.toString();
...
simpleHRM.initialize((newValue, bpm, zone) => {
...
function updateActivities(): void {
  ...
  // _steps.set(new simpleActivities.Activity(80,100));
  // _calories.set(new simpleActivities.Activity(33,100));
  // _activesminutes.set(new simpleActivities.Activity(50,100));
  // _distance.set(new simpleActivities.Activity(60,100));
